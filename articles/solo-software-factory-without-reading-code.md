...
実際、一般公開後の4週間で233本のPRをマージしました。差し戻し（revert）はゼロ本。本番環境で発生した不具合は9件でした。全停止するようなサービスダウンは、これまで一度も起きていません。
...
それでも、コードを読まない運用のまま、ひとりで安定して回し続けられています。

なぜこんな極端な開発体制を形にできたのか。理由は、5月にゼロベースで開発を始めた当初、「自分だけが使うサービス」としてスタートしたからです。

最初からユーザーがいるサービスや、たくさんの開発者がいるチーム開発では、こんなノーガード運転は怖くて真似できません。しかし、この過酷な試行錯誤から生き残った「仕様と実装のレビューループ」は、本番サービスを1人で支える確かな基盤になり、チーム開発でも明日から即効性のある武器になると実感しています。
...
それでも、開発初期における月に9件の不具合は、「人間がコードを読まないための必要経費」として受け入れられました。自分しか使っていない立ち上げ期だからこそ許された割り切りです。ここで事故を恐れずに走らせ、事故が起きるたびにガードレールを作り直してきたからこそ、有料ユーザーを抱える今になってもサービスダウンを起こさずに済んでいます。
...
守らせるために検査スクリプトを足していった結果、7月には規則が644行、検査コードだけで約7,200行に達してしまいました。また、Actionsの実行時間が月200時間を超えて課金が発生し、無料枠で回すためにリポジトリをパブリックにしてソースコードを公開するという現実的な決断も迫られました。
