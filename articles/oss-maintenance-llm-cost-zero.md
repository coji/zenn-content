...
#445ではNodeとpnpmの更新、`.github/workflows/deploy.yml`の後継アクションへの移行、minorとpatchの更新を先に固めました。
メジャー更新14件は破壊的変更を1件ずつ調べ、低リスク順に当てています。graphqlやnanoidから着手し、react-router 7から8への移行は後回しにしています。一番重かったtanstack table 8から9への移行は約25ファイルを正規APIへ移しています。`getSortedRowModel()`のような取得関数が`createSortedRowModel()`のような生成関数になり、`rowSortingFeature`などの機能登録が必要になりました。

見送り3件は理由を残して据え置きました。better-authは1.6.5に据え置きました。型推論の変更で型エラーが出たためです。parquetjsは1.8.7に据え置きました。配置の廃止で読み込みができなくなったためです。octokit系は対応待ちのためです。
