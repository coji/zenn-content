...
Artifact Share は、AIエージェントが作ったHTMLやMarkdownをURLで共有するサービスです。そのCLIに追加した `preview` というローカルコマンドを使うと、ブラウザで開いたファイルの要素をクリックするだけで指摘を書けます。あとは待つだけで、Claude Code・Codex・Cursorのいずれかがファイルを直接直し、ブラウザが自動でリロードして直った箇所を見せてくれます。実際の動きは動画のほうが早いです。

@[youtube](ooC8CCVog9A)

ここから先は、この体験を支えている仕組みの話です。サインイン・アップロードなしで動くローカル機能で、実装は公開リポジトリにあります。

https://github.com/artifactshare/artifactshare/pull/198

`preview` の完成形を図解中心で説明します。核になる技術判断は次の3つです。
...
指摘は5つの状態を、作成から終了までの流れの中で移っていきます。遷移できる主体は状態ごとに固定されていて、draft側の遷移は人間だけが行い、requested以降の前進はエージェントのCLI呼び出しだけが行います。
...
`preview next` と `preview done` というCLIの形は共通でも、「指摘が来たことをエージェントに伝える」手口は各エージェントで別物になります。誰にでも使える最低限の方法は「`preview next --wait N` をフォアグラウンドで実行し、届くまで待つ」ですが、これは待ち時間のぶんターンを塞ぎます。理想は「バッチが届いた瞬間にエージェント自身が起きる」ことですが、そのための仕組みがエージェントごとに違うため、3つとも別の実装になりました。各エージェントの挙動はCLIの公開READMEにも記載があります。
...
Cursorのagent CLIにも、Claude Codeのような自動再開の仕組みはありません。IDEのチャットに至っては、エージェントを外部から起こす手段がそもそもありません。そこでCursorは、ACP(Agent Client Protocol)経由の専用セッションを1ワークスペースにつき1つ常駐させておく方式にしました。指摘バッチが届くと、専用ランチャーが「バッチが届いた」という固定プロンプトだけをそのセッションへ送り、本文はエージェント自身が `preview next` で取りに行きます。セッションが使用中なら通知は保留され、バッチはそのまま保存されます。通常の `cursor-agent` 実行中ターンでは、環境変数でフォアグラウンドの待受コマンドに切り替えられます。素のCursor IDEチャットは自動再開の対象外で、人間が明示的に呼びかける運用になります。
...
自分のエージェントで同じ仕組みを作るなら、次の順で確認するとよいです。

- バックグラウンドタスクの完了で自動的にターンが再開するか → Claude Code方式(バックグラウンド実行+完了検知)
- 実行中セッションへ外部からイベントを注入する手段があるか → Codex方式(セッションのキューへ直接投入)
- セッションを常駐させて固定の合図を送れる、決まった受け渡し方法があるか(ACP等) → Cursor方式(管理下セッション+固定プロンプト)
- どれもなければ、フォアグラウンドで `preview next --wait N` を叩いて待つのが最後の共通手段です

`preview` の実装はソースコードごと公開されているので、この設計をそのまま自分のエージェント向けに読み替えることもできます。
