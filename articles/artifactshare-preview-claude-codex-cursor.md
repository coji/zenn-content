...
- セッションを常駐させて固定の合図を送れるプロトコルがあるか(ACP等) → Cursor方式(管理下セッション+固定プロンプト)
