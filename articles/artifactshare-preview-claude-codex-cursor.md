...
今年の5月ごろから、AIエージェントが作ったHTMLやMarkdownをURLで共有する Artifact Share というサービスを、自分で使うためにソース公開で作り続けています。そのCLIに `preview` というローカルコマンドを追加しました。ブラウザ上でファイルの要素をクリックして指摘を書くと、Claude Code・Codex・Cursor がファイルを直し、ブラウザが自動リロードで結果を見せます。動画が一番早いです。
...
- エージェントを起こす合図は、3つのエージェントで別々に作るしかなかった
...
一番苦労したのがここです。理想は「指摘が届いた瞬間にエージェントが自分で動き出す」ことですが、CLIの形は共通にできても、そのために使える仕組みがエージェントごとに違いました。3つで実際に動かして確かめた結果、こう整理できました。

- Claude Code: バックグラウンドタスクの完了でターンが自動再開する。これが一番素直だった
- Codex: 自動再開はないが、実行中セッションのキューへ外部からイベントを注入できた
- Cursor: どちらもないので、ACPでセッションを常駐させて合図を送ることにした
